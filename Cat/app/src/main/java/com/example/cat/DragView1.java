...
public class DragView1 extends android.support.v7.widget.AppCompatButton {
    private int lastX,offsetX,downX;
    private int lastY,offsetY,downY;
    private SharedPreferences.Editor editor;
    public int count=0,inst;
    //int[][] map;
    //定义监听器
    ...
    private void initView() {
       ...
       /* Resources res =getResources();
        map[0] = res.getIntArray(R.array.arry1);
        map[1] = res.getIntArray(R.array.arry2);*/
    }
...
return -1;
    ...
    public void manageoffset()
    {
        ...
        {
            Log.e("check pos", String.valueOf(pos(lastX,lastY)));
            while(lastY+offsetY>720||class_map.map[inst][pos(lastX,lastY+offsetY)+4]!=0||class_map.map[inst][pos(lastX,lastY+offsetY)+5]!=0)
            {
                if(pos(lastX,lastY)==13){
                           //class_map(this.getContext()).showbuttonnext();

                    if (mDown != null) {
                        Log.e("check pos", "beforOnDown");
                        mDown.OnDown();
                        Log.e("check pos", "afterOnDown");
                    }
                }
                    offsetY -= 240;
                    Log.e("太靠", "下  lastY:" + lastY + "  offsety:" + offsetY);
            }
            //return offsetY;
        }
        if(offsetX==0&&offsetY<0)
        {
            while(lastY+offsetY<0||class_map.map[inst][pos(lastX,lastY+offsetY)]!=0 ||class_map.map[inst][pos(lastX,lastY+offsetY)+1]!=0){offsetY+=240;Log.e("太靠", "上  lastY:"+lastY+"  offsety:"+offsetY);}
            //return offsetY;
        }
        if(offsetY==0&&offsetX<0)
        {
            while(lastX+offsetX<0||class_map.map[inst][pos(lastX+offsetX,lastY)]!=0||class_map.map[inst][pos(lastX+offsetX,lastY)+4]!=0){offsetX+=240;Log.e("太靠", "左  lastX:"+lastX+"  offsetX:"+offsetX);}
            //return offsetX;
        }
        if(offsetY==0&&offsetX>0)
        {
            while(lastX+offsetX>480||class_map.map[inst][pos(lastX+offsetX,lastY)+5]!=0||class_map.map[inst][pos(lastX+offsetX,lastY)+1]!=0){offsetX-=240;Log.e("太靠", "右  lastX:"+lastX+"  offsetX:"+offsetX);}
            //return offsetX;
        }
        //return 0;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                class_map.map[inst][pos(lastX,lastY)]--;class_map.map[inst][pos(lastX,lastY)+1]--;class_map.map[inst][pos(lastX,lastY)+4]--;class_map.map[inst][pos(lastX,lastY)+5]--;
                break;
            //单点触摸移动事件
...
/*                float tempx=lastX;
                ...
                //Log.e("保存X坐标", valueOf(getX()));
                editor.apply();
                music = MediaPlayer.create(getContext(), R.raw.cutelong);
                music.start();
                count++;
                break;
        }
