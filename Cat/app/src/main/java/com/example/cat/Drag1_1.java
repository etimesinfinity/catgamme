...
public class Drag1_1 extends android.support.v7.widget.AppCompatButton {
    private int lastX,offsetX,downX;
    private int lastY,offsetY,downY;
    private SharedPreferences.Editor editor;
    public int count=0,inst;
    private MediaPlayer music;
    public Drag1_1(Context context) {
        super(context);
        initView();
    }

    public Drag1_1(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public Drag1_1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void manageoffset()
    {
        if(offsetX==0&&offsetY>0)
        {
            ...
            while(lastY+offsetY>960||class_map.map[inst][pos(lastX,lastY+offsetY)]!=0){offsetY-=240;Log.e("太靠", "下  lastY:"+lastY+"  offsety:"+offsetY+inst);}
            //return offsetY;
        }
        if(offsetX==0&&offsetY<0)
        {
            while(lastY+offsetY<0||class_map.map[inst][pos(lastX,lastY+offsetY)]!=0){if(class_map.map[inst][pos(lastX,lastY+offsetY)]!=0)Log.e("map", "wrong");offsetY+=240;Log.e("太靠", "上  lastY:"+lastY+"  offsety:"+offsetY);}
            //return offsetY;
        }
        ...
        if(offsetY==0&&offsetX>0)
        {
            while(lastX+offsetX>720||class_map.map[inst][pos(lastX+offsetX,lastY)]!=0){offsetX-=240;Log.e("太靠", "右  lastX:"+lastX+"  offsetX:"+offsetX);}
            //return offsetX;
        }
        //return 0;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                class_map.map[inst][pos(lastX,lastY)]--;
                break;
            //单点触摸移动事件
