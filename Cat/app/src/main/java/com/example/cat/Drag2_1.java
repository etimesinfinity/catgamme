...
public class Drag2_1 extends android.support.v7.widget.AppCompatButton {
    private int lastX,offsetX,downX;
    private int lastY,offsetY,downY;
    private SharedPreferences.Editor editor;
    public int count=0,inst;
    private MediaPlayer music;
    public Drag2_1(Context context) {
        super(context);
        initView();
    }

    public Drag2_1(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public Drag2_1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void manageoffset()
    {
        if(offsetX==0&&offsetY>0)
        {
            ...
            while(lastY+offsetY>960||class_map.map[inst][pos(lastX,lastY+offsetY)]!=0||class_map.map[inst][pos(lastX,lastY+offsetY)+1]!=0){offsetY-=240;Log.e("太靠", "下  lastY:"+lastY+"  offsety:"+offsetY);}
            //return offsetY;
        }
        if(offsetX==0&&offsetY<0)
        {
        ...
        {
            while(lastX+offsetX<0||class_map.map[inst][pos(lastX+offsetX,lastY)]!=0){offsetX+=240;Log.e("太靠", "左  lastX:"+lastX+"  offsetX:"+offsetX);}
            //return offsetX;
        }
        if(offsetY==0&&offsetX>0)
        {
            while(lastX+offsetX>480||class_map.map[inst][pos(lastX+offsetX,lastY)+1]!=0){offsetX-=240;Log.e("太靠", "右  lastX:"+lastX+"  offsetX:"+offsetX);}
            //return offsetX;
        }
        //return 0;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                class_map.map[inst][pos(lastX,lastY)]--;class_map.map[inst][pos(lastX,lastY)+1]--;
                break;
            //单点触摸移动事件
